...
Within one outcome fragment, a row key is: confirmed dependency + failure type (timeout / DNS failure / authentication failure / partial outage) + production entrypoint + scenario (West US 2 to West US regional failover). Separate distinct outcomes or distinct priorities into distinct rows. Retain every causal citation on the row it belongs to.
